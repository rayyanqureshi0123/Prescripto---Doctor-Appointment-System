...
const Appointment = () => {
  ...
  // Fetch doctor info (can later be replaced with API call)
...
while (currentDate < endTime) {
  ...
  const isSlotAvailable = bookedSlots[slotDate]?.includes(slotTime) ? false : true;
  ...
  return docInfo && (
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        <div>
          <img className="bg-[#5f6FFF] w-full sm:72 rounded-lg " src={docInfo.image} alt="" />
        </div>
        <div className="flex-1 border border-gray-400 rounded-lg p-8 py-7 bg-white mx-0 mt-[-80px] sm:mt-0 ">
          ...
          <p className="flex items-center gap-2 text-2xl font-medium text-gray-900 ">
            {docInfo.name} <img className="w-5" src={assets.verified_icon}></img>
          </p>
